...
""")
    ...
    if analyze_flip:
        ...
        # Flip Deal Score (0–100)
        # 40% ROI target 25%, 30% Profit Margin target 20%, 30% Net Profit target $50k
        flip_score_raw = (
            (roi / 0.25 * 40) +
            (profit_margin / 0.20 * 30) +
            (net_profit / 50000 * 30)
        )
        flip_score = max(0, min(100, flip_score_raw))

        flip_rating = (
            "🔥 Excellent Flip" if flip_score >= 85 else
            "✅ Strong Flip" if flip_score >= 70 else
            "⚠️ Marginal Flip" if flip_score >= 50 else
            "❌ Weak Flip"
        )
        ...
        st.session_state.flip_results = {
            ...
            "ARV": flip_arv,
            "Holding Months": flip_holding_months,
            "Financing Costs": flip_financing_costs,
            "Misc / Contingency": flip_misc_costs,
            "Selling Cost %": flip_selling_cost_pct,
